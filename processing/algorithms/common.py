...
def smooth_ppi_rays(ppi_data, n):
    ...
    ppi_cs = scipy.sparse.hstack(
        (zero_mat, ppi_data.reshape((zero_mat.shape))))
...
def beam_blocking(radar, srtm_ffn):
    ...
    try:
        ...
        print('beamwidth info missing from volume, using default of 0.925 deg')
        bw = 1
    ...
    for tilt, el in enumerate(el_list):
        ...
        nrays = radar_ccb[sweep_idx].shape[0]
        ...
        rastervalues, rastercoords, _ = wrl.georef.extract_raster_dataset(
            ds, nodata=-32768.)
